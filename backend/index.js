...
app.post('/add_product', upload.single('event_image'), async (req, res) => {
  ...
  try {
    ...
    if (result) {
      ...
      return res.status(500).json({ message: 'Error occurred while saving event dataaaaa' });
    }
  } catch (err) {
    if (err.name === 'ValidationError') {
      return res.status(400).json({ message: 'Invalid data formattt' });
    } else {
      return res.status(500).json({ message: 'Errorss occurred while saving event data' });
    //dwedkl
  }
//ferkfoerfjeiorfjeredewd
   }
});
